...
class Denoiser(nn.Module):
    ...
    输入：加噪的Steps Embedding + 时间步 + Query条件 + Self-Conditioning
    输出：预测的噪声（或x0）
    
    Self-Conditioning: 将上一步预测的x0作为额外条件输入
    """
    ...
    def __init__(
        ...
        # Self-Conditioning: x0预测值的投影层
        self.x0_proj = nn.Sequential(
            nn.Linear(hidden_size, hidden_size),
            nn.SiLU(),
            nn.Linear(hidden_size, hidden_size),
        )
        # 学习的空条件向量（当没有x0_cond时使用）
        self.x0_null = nn.Parameter(torch.zeros(1, 1, hidden_size))
    ...
    def forward(
        ...
        Args:
            ...
            x0_cond: Self-Conditioning条件，上一步预测的x0 [B, L, H]，可为None
            
        Returns:
            predicted_noise: 预测的噪声（或x0） [B, L, H]
        """
        ...
        # 3. Self-Conditioning: 处理x0条件
        if x0_cond is not None:
            # 使用上一步预测的x0作为额外条件
            x0_emb = self.x0_proj(x0_cond)  # [B, L, H]
        else:
            # 使用学习的空条件向量
            x0_emb = self.x0_null.expand(batch_size, seq_len, -1)

        # 4. 输入处理：融合x_t和x0_cond
        x = self.input_proj(x_t) + x0_emb  # [B, L, H]
