...
class LitCoLaR(LitCoTModelBase):
    ...
    def forward(self, batch):
        ...
        if r == 1:  # 不压缩，直接使用原始 embeddings
            steps_inputs_embeds = self.embedding(steps_input_ids)
            steps_labels = steps_input_ids  # 标签就是原始 token ids
        else:
            ...
            # 2. 计算需要额外添加多少左 padding
            # 目标：让第一个真实 token (###) 的位置是 r 的倍数
            ...
            for b, l_length in enumerate(n_extra_left_pad_length):
                ...
                # 为避免左侧增加padding导致无法被r整除，所以要在右侧增加padding
                ...
                # 取出第 b 个样本的 token ids 和 attention mask
                s_ids = steps_input_ids[b]
                s_attn_mask = steps_attention_mask[b]

                if l_length > 0:
                    # 创建 l_length 个 pad token id，拼接到 s_ids 的左边
                    s_ids = torch.cat(
                        [
                ...
                if r_length > 0:
                    ...
                    # 创建 r_length 个 0，拼接到 attention_mask 的右边
                    s_attn_mask = torch.cat(
            ...
            # reshape: (batch, 15, hidden) → (batch, 5, 3, hidden)
            # 然后在 dim=2 上求和，得到 (batch, 5, hidden)
            ...
            ).sum(dim=2)  # 每组中有多少个真实 token

            # 10. 计算平均（除以每组的真实 token 数）
            if latent_cot_config.get("sqrt_mean", False):
                compressed_steps_attention_mask = compressed_steps_attention_mask.sqrt()
            compressed_steps_inputs_embeds /= compressed_steps_attention_mask.unsqueeze(-1) + 1e-5  # +1e-5 防止除零
            
            # 11. 更新 attention_mask（有真实 token 的组标记为 1）
